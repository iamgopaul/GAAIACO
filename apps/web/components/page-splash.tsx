...
// Stage timelines (ms). Stages: 0 orb · 1 rings · 2 wordmark · 3 welcome ·
// 4 mission · 5 fade-out. First load runs the whole thing slowly; navigations
// just show the logo icon with the beating orb, then reveal.
const FIRST_LOAD = {
  rings: 2800,
  wordmark: 4800,
  welcome: 7200,
...
export function PageSplash() {
  ...
  // Skip: dismiss the splash immediately and cancel any pending stage timers.
  ...
  // Drive the timeline. Keyed on pathname so it re-runs on every navigation.
  useEffect(() => {
    setMounted(true);
    ...
    if (isFirst) {
      ...
      // Navigation / refresh: logo icon with the beating orb, then reveal.
  ...
  // Any keypress skips the splash. (Clicks are handled on the overlay itself.)
  useEffect(() => {
    const onKey = () => skip();
  ...
  // Each element is mounted only while it's the active stage, then unmounted —
  // so nothing lingers as a ghost behind the next thing.
        ...
        <div className="gaaia-splash-stage">
          ...
          {showWelcome && (
            <p className="gaaia-fade-in gaaia-intro-headline">
              Welcome to the
              <br />
              <span className="text-silver">GAAIA Ecosystem</span>
            </p>
          )}

          {showMission && (
            <div className="gaaia-fade-in flex flex-col items-center gap-5">
              <span className="gaaia-intro-sub">
                Global Autonomous Artificial Intelligence Agency
              </span>
              <p className="gaaia-intro-headline">
                We build{" "}
